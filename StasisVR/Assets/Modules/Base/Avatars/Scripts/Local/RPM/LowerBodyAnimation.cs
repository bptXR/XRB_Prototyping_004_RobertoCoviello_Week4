...
namespace BearingPoint.Avatars.Local.RPM
{
    public class LowerBodyAnimation : MonoBehaviour
    {
        ...
        private void OnAnimatorIK(int layerIndex)
        {
            ...
            bool isRightFootDown = Physics.Raycast(
                rightFootPosition + raycastRightOffset,
                Vector3.down,
                out hitRightFoot);
        ...
        private void CalculateLeftFoot(bool isLeftFootDown, RaycastHit hitLeftFoot)
        {
            if (isLeftFootDown)
            {
                ...
                Quaternion leftFootRotation = Quaternion.LookRotation(
                    Vector3.ProjectOnPlane(transform.forward, hitLeftFoot.normal),
                    hitLeftFoot.normal);
        ...
        private void CalculateRightFoot(bool isRightFootDown, RaycastHit hitRightFoot)
        {
            if (isRightFootDown)
            {
                ...
                Quaternion rightFootRotation = Quaternion.LookRotation(
                    Vector3.ProjectOnPlane(transform.forward, hitRightFoot.normal),
                    hitRightFoot.normal);
